...
**Summary**: 모델 라우팅은 태스크의 복잡도와 특성에 따라 적절한 Claude 모델(Opus/Sonnet/Haiku)을 자동으로 선택하여 품질과 비용을 최적화하는 전략이다. [[subagents|서브에이전트]] frontmatter의 `model` 필드로 설정한다.
...
모든 태스크에 가장 강력한 모델을 사용하면 불필요한 비용이 발생하고, 모든 태스크에 가장 빠른 모델을 사용하면 품질이 저하된다. [[claude-code-subagents|VoltAgent]] 컬렉션은 각 서브에이전트에 `model` 필드를 포함하여 태스크 유형에 맞는 모델로 자동 라우팅한다. (source: VoltAgentawesome...)
...
`model: inherit` 설정 시 주 대화에서 사용 중인 모델을 그대로 사용한다. 개발자가 에이전트 파일의 `model` 필드를 직접 수정하여 기본값을 오버라이드할 수 있다.
